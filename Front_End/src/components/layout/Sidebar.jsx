...
import {
  LayoutDashboard, BookOpen, Users, CreditCard, BookCopy,
  FileText, Settings, LogOut, ChevronLeft, ChevronRight, DollarSign,
  BookMarked, BarChart3,
} from 'lucide-react';
  ...
  return (
    ...
    >
      ...
      <div className={`flex items-center h-16 px-4 border-b border-gray-100 ${collapsed ? 'justify-center' : 'gap-3'}`}>
        {/* Đổi Logo icon sang màu Cam/Đất của chữ Libra */}
      ...
      <nav className="flex-1 py-4 px-2 overflow-y-auto space-y-1">
        {navItems.map(({ to, icon: Icon, label }) => (
          <NavLink
            ...
            to={to}
            className={({ isActive }) =>
              // Đổi highlight khi active sang màu be/xanh đen
              `flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors
              ${isActive ? 'bg-[#F4EFE7] text-[#354336]' : 'text-gray-600 hover:bg-gray-50 hover:text-[#354336]'}
